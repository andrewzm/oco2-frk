...
## Compare FRK V7 or V8 (both without target)
compare_from <- "FRKv8_no_target" # Can be FRKv7_no_target OR FRKv8_no_target
...
## Choose dir based on FRK product we are comparing
...
if(save_images) {
    ...
    ## Find the slop constrained to pass through the origin
    ...
    for(name_station in c("Lamont","Wollongong")) {
        ...
        g1 <- ggplot(X) + geom_point(aes(date,oco2_xco2)) +
            ...
            geom_errorbar(aes(date,
                              ...
                              ymax = oco2_xco2 + oco2_sd),alpha=0.7,col="black") +
